...
このアプリの役割:
  - 全24会場を横断して締切間際のレースをスキャンし、荒れ度スコアを画面表示する
  - 手動でLINEにテスト通知を送れる
  - バックグラウンドの自動通知は同リポジトリの GitHub Actions
    （.github/workflows/rough_race_notify.yml が main.py を定期実行）が担当し、
    このダッシュボードを開いていなくても自動で動く
...
st.caption(
    "全24会場を横断し、締切15分前後のレースを展示タイム・周回タイムから荒れ度スコアで判定します。"
...
with col1:
    scan_clicked = st.button("🔍 今すぐスキャン", type="primary", use_container_width=True)
...
st.divider()
st.caption(
    "このアプリは検出ロジックの手動確認・手動送信用です。定期的な自動監視とLINE通知は "
